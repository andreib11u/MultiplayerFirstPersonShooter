...
UAbilityTask_WaitTargetDataUsingActor::UAbilityTask_WaitTargetDataUsingActor(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UAbilityTask_WaitTargetDataUsingActor* UAbilityTask_WaitTargetDataUsingActor::WaitTargetDataWithReusableActor(
	UGameplayAbility* OwningAbility, FName TaskInstanceName, TEnumAsByte<EGameplayTargetingConfirmation::Type> ConfirmationType,
	AGameplayAbilityTargetActor* InTargetActor, bool bCreateKeyIfNotValidForMorePredicting)
{
	UAbilityTask_WaitTargetDataUsingActor* MyObj = NewAbilityTask<UAbilityTask_WaitTargetDataUsingActor>(
...
void UAbilityTask_WaitTargetDataUsingActor::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& Data)
{
	...
	FScopedPredictionWindow ScopedPrediction(
		AbilitySystemComponent.Get(), ShouldReplicateDataToServer() && (bCreateKeyIfNotValidForMorePredicting &&
...
void UAbilityTask_WaitTargetDataUsingActor::ExternalConfirm(bool bEndTask)
{
	check(AbilitySystemComponent.IsValid());
	if (TargetActor)
	{
...
void UAbilityTask_WaitTargetDataUsingActor::ExternalCancel()
{
	check(AbilitySystemComponent.IsValid());
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
void UAbilityTask_WaitTargetDataUsingActor::InitializeTargetActor() const
{
	...
	TargetActor->TargetDataReadyDelegate.AddUObject(const_cast<UAbilityTask_WaitTargetDataUsingActor*>(this),
													&UAbilityTask_WaitTargetDataUsingActor::OnTargetDataReadyCallback);
	TargetActor->CanceledDelegate.AddUObject(const_cast<UAbilityTask_WaitTargetDataUsingActor*>(this),
											 &UAbilityTask_WaitTargetDataUsingActor::OnTargetDataCancelledCallback);
}
	...
	if (!bIsLocallyControlled)
	{
		...
		if (!bShouldProduceTargetDataOnServer)
		{
			...
			AbilitySystemComponent->AbilityTargetDataSetDelegate(SpecHandle, ActivationPredictionKey)
				.AddUObject(this, &UAbilityTask_WaitTargetDataUsingActor::OnTargetDataReplicatedCallback);
			AbilitySystemComponent->AbilityTargetDataCancelledDelegate(SpecHandle, ActivationPredictionKey)
				.AddUObject(this, &UAbilityTask_WaitTargetDataUsingActor::OnTargetDataReplicatedCancelledCallback);
...
void UAbilityTask_WaitTargetDataUsingActor::OnDestroy(bool AbilityEnded)
{
	if (TargetActor)
	{
